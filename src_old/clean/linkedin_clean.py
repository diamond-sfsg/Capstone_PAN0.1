...
XLSX_PATH = RAW_DIR / "sp500_linkedin_current_info_FINAL.xlsx"
OUTPUT_PATH = Path("data/clean_2.0/linkedin_clean_v4.jsonl")
...
def read_xlsx_table(path: Path) -> pd.DataFrame:
    """
    Read the LinkedIn workbook without requiring openpyxl in the project env.
    The updated raw LinkedIn data is a simple one-sheet xlsx export.
    """
    if not path.exists():
        return pd.DataFrame()
    ...
    header = [clean_text(value) for value in matrix[0]]
    data = matrix[1:]
...
def build_records_from_workbook(path: Path) -> List[Dict]:
    records = []
    df = read_xlsx_table(path)
    if df.empty:
        return records

    for idx, row in df.iterrows():
        status = clean_text(row.get("extraction_status", ""))
        if status and status.lower() != "success":
            continue

        company = normalize_company_name(str(row.get("display_company_name") or row.get("company") or ""))
        if not company:
            continue

        about_text = clean_text(row.get("about_text", ""))
        ...
        raw_text_sample = clean_html_to_text(str(row.get("raw_text_sample", "") or ""))
        ...
        text_parts = [
            ...
            f"Specialties: {specialties}" if specialties else "",
            raw_text_sample,
        ]
        combined_text = clean_text(" ".join(part for part in text_parts if part))
        ...
        for chunk_idx, chunk in enumerate(chunks):
            records.append({
                ...
                "chunk_id": f"{company}_linkedin_company_page_{chunk_idx}",
                "company": company,
                "ticker": None,
                "year": None,
                "source": "linkedin",
                ...
                "section": None,
                "page_title": clean_text(row.get("page_title", "")) or None,
                "url": clean_text(row.get("linkedin_url", "")) or None,
                "text": chunk,
                "metadata": {
                    "source_workbook": path.name,
                    "row_index": int(idx),
                    "followers_number": clean_text(row.get("followers_number", "")) or None,
                    "employees_number": clean_text(row.get("employees_number", "")) or None,
...
def run_pipeline():
    ...
    if XLSX_PATH.exists():
        print(f"[INFO] Processing LinkedIn workbook: {XLSX_PATH.name}")
        all_records.extend(build_records_from_workbook(XLSX_PATH))
    else:
